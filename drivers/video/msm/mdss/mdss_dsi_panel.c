...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if(ctrl->read_manufacture_cmds.cmd_cnt)
	{
		printk(KERN_ERR"Start read the lcd register value \n");
		mdss_oem_read_reg(ctrl, &ctrl->read_manufacture_cmds);
	}
		...
		struct dsi_panel_cmds *pcmds, char *cmd_key, char *link_key)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		if (read_cmd) {
			...
			len -= 2;
			pr_err("%s address : 0x%x size : 0x%x offset : 0x%x\n", __func__,
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->read_manufacture_cmds,
			"qcom,mdss-dsi-id-command", "qcom,mdss-dsi-id-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->read_reg_enable,
			"qcom,mdss-dsi-read-reg-enable-command", "qcom,mdss-dsi-read-reg-enable-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->read_reg_disable,
			"qcom,mdss-dsi-read-reg-disable-command", "qcom,mdss-dsi-read-reg-disable-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->packet_size_tx_cmds,
		"qcom,mdss-dsi-packetsizetxcmds-command", "qcom,mdss-dsi-packetsizetxcmds-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->read_pos_tx_cmds,
		"qcom,mdss-dsi-readpostxcmds-command", "qcom,mdss-dsi-readpostxcmds-command-state");
