...
} bootloader_log_ram_zone_t;


static bootloader_log_ram_zone_t bootloader_log_ram_zone;
...
static int persistent_ram_buffer_map(phys_addr_t start, size_t size,
		bootloader_log_ram_zone_t *blrz)
{
	blrz->paddr = start;
	blrz->size = size;
...
static int bootloader_log_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		if (of_bootloader_log_platform_data(pdev->dev.of_node, &of_pdata)) {
	...
	err = persistent_ram_buffer_map(paddr, pdata->mem_size, &bootloader_log_ram_zone);
	if (err)
		goto fail_out;
