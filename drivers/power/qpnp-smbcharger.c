...
void charger_type_record(struct work_struct *work)
{
    ...
    if(is_find_key_word) {
		...
		sprintf(index,"|%d",chip->record_chg_type_count);
		strcat(param_value, index);
        ...
        pr_info("%s: charger_type_record: param_value = %s,dash_on=%d,dash_present=%d\n",__func__,param_value,dash_on[0],chip->dash_present);
